...
import { updateTranslateData, translateFieldNameSort, getContentCount } from './helper/index.js';
...
import { selectGlossaryTerms, selectTranslatePostInfo } from './redux-store/features/selectors.js';
...
export const updateContent = async ({ source, postId, sourceLang, lang, editorType, createTranslatePostNonce, storeDispatch }) => {
    ...
    const reTranslationData = selectTranslatePostInfo(store.getState())?.[postId + '_' + lang]?.Retranslate;
    const Retranslate = reTranslationData?.status === true;
    const reTranslateBody={reTranslate:{status:false}};

    if (Retranslate) {
        reTranslateBody.reTranslate = {status:true};
        if(Retranslate && reTranslationData?.reTranslateContent && Object.keys(reTranslationData?.reTranslateContent).length > 0){
            const reTranslationFieldTypes=translateFieldNameSort(Object.keys(reTranslationData?.reTranslateContent));

            reTranslateBody.reTranslate.fieldsType=reTranslationFieldTypes;
            
            if(reTranslationFieldTypes.length > 0){
                Object.keys(deepCloneSource).forEach(key=>{
                    if(!reTranslationFieldTypes.includes(key)){
                        delete deepCloneSource[key];
                    }
                })

                if(reTranslationData.reTranslateContent.metaFields === true && reTranslationData.reTranslateContent.metaFieldsData){
                    const metaFieldsKeys=Object.keys(reTranslationData.reTranslateContent.metaFieldsData);

                    if(deepCloneSource.metaFields && Object.keys(deepCloneSource.metaFields).length > 0){
                        Object.keys(deepCloneSource.metaFields).forEach(key=>{
                            if(!metaFieldsKeys.includes(key)){
                                delete deepCloneSource.metaFields[key];
                            }
                        });
                    }
                }
            }else{
                return console.warn('Retranslate is true but reTranslationData Fields is empty');
            }
        }else{
            return console.warn('Retranslate is true but reTranslationData is empty');
        }

        if(!reTranslationData.currentPostId){
            return console.warn('Retranslate is true but currentPostId is empty');
        }

        const reTranslateId=reTranslationData.currentPostId;

        reTranslateBody.reTranslate.postId=reTranslateId;

        const reTranslatePrivateKey=reTranslationData.validationKey;

        if(!reTranslatePrivateKey || '' === reTranslatePrivateKey){
            return console.warn('Retranslate is true but reTranslate Key not found');
        }

        reTranslateBody.reTranslate.privateKey=reTranslatePrivateKey;
    }
    ...
    let body = {
        ...
        reTranslateData: JSON.stringify(reTranslateBody.reTranslate)
    }
    ...
    if (editorType === 'taxonomy') {
        ...
        body.meta_fields = updateContent.metaFields ? JSON.stringify(updateContent.metaFields) : '';
...
const bulkTranslateEntries = async ({ ids, langs, storeDispatch }) => {
    ...
    if (postKeys.length > 0) {
        ...
        postKeys.forEach(postId => {
            ...
            const reTranslatePosts=posts[postId].ReTranslatePosts;
            ...
            if(reTranslatePosts && Object.keys(reTranslatePosts).length > 0){
                Object.keys(reTranslatePosts).forEach(postId => {
                    const reTranlsateStatus=reTranslatePosts[postId]?.re_translation_status === true;
                    const language = reTranslatePosts[postId]?.language;
                    const titleTranslate=reTranslatePosts[postId]?.re_translate_title === true;
                    const excerptTranslate=reTranslatePosts[postId]?.re_translate_excerpt === true;
                    const contentTranslate=reTranslatePosts[postId]?.re_translate_content === true;
                    const metaFieldsTranslate=reTranslatePosts[postId]?.re_translate_metaFields === true;
                    const reTranslateMetaFields=reTranslatePosts[postId]?.metaFields;
                    const parentPostId=reTranslatePosts[postId]?.parent_post_id;
                    const validationKey=reTranslatePosts[postId]?.validation_key;

                    if(!language || typeof language !== 'string'){
                        return;
                    }

                    if(!reTranlsateStatus){
                        return;
                    }

                    let firstPostLanguage = false;

                    if (!postIdExist.includes(parentPostId) && !existsPostInPendingPosts.includes(parentPostId + '_' + language)) {
                        postIdExist.push(parentPostId);
                        firstPostLanguage = true;
                    }
                    ...
                    if(reTranlsateStatus){
                       Retranslate.status=true; 
                    }

                    if(titleTranslate){
                        reTranslateContent.title=true;
                    }

                    if(excerptTranslate){
                        reTranslateContent.excerpt=true;
                    }

                    if(contentTranslate){
                        reTranslateContent.content=true;
                    }

                    if(metaFieldsTranslate){
                        reTranslateContent.metaFields=true;

                        if(reTranslateMetaFields){
                            reTranslateContent.metaFieldsData=reTranslateMetaFields;
                        }
                    }

                    if(validationKey && '' !== validationKey){
                        Retranslate.validationKey=validationKey;
                    }

                    if(Object.keys(Retranslate).length < 1 || Object.keys(reTranslateContent).length < 1){
                        return;
                    }

                    Retranslate.currentPostId=postId;
                    Retranslate.reTranslateContent=reTranslateContent;

                    const flagUrl = lmatBulkTranslationGlobal.languageObject[language].flag;
                    const languageName = lmatBulkTranslationGlobal.languageObject[language].name;
                    storeDispatch(updatePendingPosts([parentPostId + '_' + language]));
                    storeDispatch(updateTranslatePostInfo({ [parentPostId + '_' + language]: { parentPostId, targetPostId: null, targetLanguage: language, postLink: null, status: 'pending', parentPostTitle, firstPostLanguage, flagUrl, languageName, messageClass: 'warning', Retranslate } }));
                });
            }
        });
        ...
        const fetchGlossaryTerms = async (allTargetLanguages) => {
            ...
            const fethcGlossary = async (sourceLanauges) => {
                ...
                try {
                    ...
                    const filterdTerms = Object.values(responseData.data.terms).map(term => { return { ...term, original_term: term.original_term.toLowerCase() } });

                    if (responseData.success && responseData.data.terms) {
                        storeDispatch(updateGlossaryTerms({ sourceLanguage: sourceLanauges, translations: filterdTerms }));
                    }
                } catch (err) {
                    console.log(err);
                }
        ...
        const storeSourceContent = async (index, translatePostsCount) => {
            ...
            const { title, content, post_name, languages, editor_type, metaFields = null, sourceLanguage, excerpt = null, ReTranslatePosts } = posts[postId];
            ...
            const reTranslatePost=ReTranslatePosts && Object.keys(ReTranslatePosts).length > 0;
            if (languages && languages.length > 0 || reTranslatePost) {
                let reTranslateLang=[];
                
                if(reTranslatePost){
                    reTranslateLang=Object.values(ReTranslatePosts).map(reTranslate => reTranslate.language);
                }

                let targetLang=languages;

                if(targetLang){
                    targetLang.push(...reTranslateLang);
                }else{
                    targetLang=reTranslateLang;
                }
