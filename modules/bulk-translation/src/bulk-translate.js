            ...
            storeDispatch(
                updateTranslatePostInfo({
                    [pendingKey]: {
                        ...
                        status: 'error',
                        messageClass: 'error',
                        errorMessage: __('API quota exceeded (429). Please check your plan/billing and retry later.', 'linguator-multilingual-ai-translation'),
                        errorHtml: false,
                    },
                })
            );
    ...
    await fetch(bulkTranslateRouteUrl + `/${postId}:${endPoint}`, {
       ...
       // Some server/proxy errors return HTML (e.g. 502/504). Don't blindly JSON.parse.
        ...
        if (successPayload){
            ...
            successPayload.post_title = '' === successPayload.post_title ? __('N/A', 'linguator-multilingual-ai-translation') : successPayload.post_title;
