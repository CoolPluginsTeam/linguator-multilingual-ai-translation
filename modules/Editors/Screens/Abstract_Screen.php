...
abstract class Abstract_Screen {
	...
	public function enqueue(): void {
		...
		$handle     = static::get_handle();
		$script_url = plugins_url( $this->get_script_path(), LINGUATOR_ROOT_FILE );

		// Load dependency list generated by DependencyExtractionWebpackPlugin, if available.
		$asset_path = plugin_dir_path( LINGUATOR_ROOT_FILE ) . ltrim( str_replace( '.js', '.asset.php', $this->get_script_path() ), '/' );
		$asset      = array(
			'dependencies' => array(),
			'version'      => LINGUATOR_VERSION,
		);
		if ( file_exists( $asset_path ) ) {
			$loaded_asset = include $asset_path; // returns [ 'dependencies' => [], 'version' => '...' ]
		...
		$dependencies = array_values( array_unique( array_merge( $base_dependencies, $asset['dependencies'] ?? array() ) ) );

		wp_enqueue_script(
			$handle,
			$script_url,
			$dependencies,
			$asset['version'] ?? LINGUATOR_VERSION,
			true
		);
