 ...
 * @param {Object} fields - An object containing the fields to be translated.
 ...
 */
export const getTranslationEntries = (state, fields=false) => {
    // Initialize an empty array to hold translation entries
    const translateEntry = new Array;

    if (state.title && state.title.source && (!fields || fields.title === true)) {
        const titleData = {
    ...
    if (state.excerpt && state.excerpt.source && (!fields || fields.excerpt === true)) {
        const excerptData = {
    ...
    if (state.slug && state.slug.source && (!fields || fields.slug === true)) {
        const slugData = {
    ...
    if((!fields || fields.metaFields === true)){
...
export const getTranslationInfo = (state) => {
    ...
    if(state.title && state.title.source && state.title.source !== '') {
        data.availableContentTypes.title=true;
    }

    if(state.excerpt && state.excerpt.source && state.excerpt.source !== '') {
        data.availableContentTypes.excerpt=true;
    }

    if(state.slug && state.slug.source && state.slug.source !== '') {
        data.availableContentTypes.slug=true;
    }

    if(state.content && Object.keys(state.content).length > 0) {
        data.availableContentTypes.content=true;
    }
    
    if(state.metaFields && Object.keys(state.metaFields).length > 0) {
        data.availableContentTypes.metaFields=true;
    }
...
export const getReTranslationFields = (state) => {
    return state.reTranslationFields;
}