...
if (!class_exists('LMAT_cronjob')) {
    class LMAT_cronjob
    {
        ...
        function lmat_cron_extra_data_autoupdater() {
            ...
            // Get user info directly without instantiating the class
            $extra_data_details = self::get_user_info();
            
            $server_info    = $extra_data_details['server_info'];
            $extra_details  = $extra_data_details['extra_details'];
            $site_url       = get_site_url();
            $install_date   = get_option('lmat-install-date');
            $uni_id         = '53';
            $site_id        = $site_url . '-' . $install_date . '-' . $uni_id;
            $initial_version = get_option('lmat_initial_save_version');
            $initial_version = is_string($initial_version) ? sanitize_text_field($initial_version) : 'N/A';
            $plugin_version = defined('LINGUATOR_VERSION_CURRENT') ? LINGUATOR_VERSION_CURRENT : 'N/A';
